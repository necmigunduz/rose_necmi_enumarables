...
#Task 4 my_all
  def my_all
    if block_given? 
    result=Array.new
    (0...self.length).each {|element| result.push(yield(self[element]))}
...
# Task-6 my_count
  def my_count
    if block_given?
      count = 0
      (0...self.length).each {|element| count += 1 if yield(element)}
      p count
    else
      to_enumerator(:count)
    end
...
# Task-7 my_map
  def my_map
    if block_given?
      animal = Array.new
      self.length.times {|element| animal.push(yield)}
      p animal
    else
